...
/**
 * Hexi's body — an original little hextech spirit (SVG, CSS animated).
 * Shared by the sidebar mascot and the in-game overlay.
 */
export function HexiSprite(props: {
  ...
  return (
    <svg
      viewBox="0 0 64 64"
      className={`hexi ${props.className ?? 'h-16 w-16'} ${alerting ? 'gold-pulse rounded-full' : ''}`}
      data-mood={alerting ? 'hyped' : props.mood}
      role="img"
...
/**
 * Hexi in the navigation: mood follows the session phase; speaks a short
 * phrase on phase change and squeaks visually when a spike alert lands.
 */
